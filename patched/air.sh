...
$HOME_WFB/wfb_tx -K $HOME_WFB/drone.key -p 7 -u 5800 $wl > /dev/null 2>&1 &
echo $! >> $PIDFILE
$HOME_WFB/wfb_tx -K $HOME_WFB/drone.key -p 6 -u 5700 $wl > /dev/null 2>&1 &
echo $! >> $PIDFILE
$HOME_WFB/wfb_tx -K $HOME_WFB/drone.key -p 1 -u 5600 $wl > /dev/null 2>&1 &
echo $! >> $PIDFILE
...
if uname -a | grep -cs "4\.9"> /dev/null 2>&1;then
  DEVICE="/dev/ttyTHS1"
  $HOME_PRJ/scripts/air_camjet.sh $PIDFILE
else
  DEVICE="/dev/ttyAMA0"
  $HOME_PRJ/scripts/air_campi.sh
fi

# TO BE DONE : GET ALL PIDS
