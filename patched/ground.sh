...
#  $HOME_WFB/wfb_rx -K $HOME_WFB/gs.key -p 7 -u $((5800+nb)) -c $GCS_IP $wl > /dev/null 2>&1 &
#  echo $! >> $PIDFILE
#  $HOME_WFB/wfb_rx -K $HOME_WFB/gs.key -p 6 -u $((5700+nb)) -c $GCS_IP $wl > /dev/null 2>&1 &
#  echo $! >> $PIDFILE
  $HOME_WFB/wfb_rx -K $HOME_WFB/gs.key -p 1 -u $((5600+nb)) -c $GCS_IP $wl > /dev/null 2>&1 &
  echo $! >> $PIDFILE
  if [[ $nb = 1 ]]; then
  ...
  echo $! >> $PIDFILE
#  $HOME_WFB/wfb_rx -K $HOME_WFB/gs.key -p 6 -u 4250 -c 127.0.0.1 -k 1 -n 2 $wl > /dev/null 2>&1 &
#  echo $! >> $PIDFILE
